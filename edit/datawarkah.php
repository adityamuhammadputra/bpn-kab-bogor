    ...
    <div class="container body">
      <div class="main_container">
        ...
        <div class="right_col" role="main">
          ...
          <?php 
            $id = $_GET['id'];
            $q = "SELECT * FROM tbl_datawarkah WHERE id='".$id."'";
            $s = mysqli_query($conn, $q);
            $d = mysqli_fetch_array($s);
          ?>                                
          <div class="container pad">
            <form role="form" method="post" class="col-md-12" action="../proses/datawarkah.php"  enctype="multipart/form-data">
              <div class="col-md-6">
                <input type="text" name="id" value="<?php echo $id; ?>" hidden="true"/>
                <input type="text" name="inputan" value="<?php echo $_SESSION['username']; ?>" hidden="true"/>
                <div class="form-group">
                <label>Warkah Nomor</label>
                <input class="form-control" type="text" name="no_warkah" value="<?php echo $d['no_warkah']; ?>"/>
                </div>
                <div class="form-group">
                  <label>Nomor Box</label>
                  <input class="form-control" type="text" name="no_box" value="<?php echo $d['no_box']; ?>" />
                </div>
                <div class="form-group">
                  <label>Tahun</label>
                  <input class="date-own form-control" type="text" name="tahun" value="<?php echo $d['tahun']; ?>" />
                </div>
                <div class="form-group">
                  <label>Lokasi Ruang</label>
                  <input class="form-control" type="text" name="lokasi_ruang" value="<?php echo $d['lokasi_ruang']; ?>"/>
                </div>
              </div>
              <div class="col-md-6">
                <div class="form-group">
                <label>Rak</label>
                <input class="form-control" type="text" name="rak" value="<?php echo $d['rak']; ?>"/>
                </div>
                <div class="form-group">
                  <label>Baris</label>
                  <input class="form-control" type="text" name="baris" value="<?php echo $d['baris']; ?>" />
                </div>
                <div class="form-group">
                  <label>Posisi</label>
                ...
                <div class="form-group">
                  <label>Foto</label>
                  <input class="form-control" type="file" name="foto" value="<?php echo$d['foto']; ?>" />
                  <input type="checkbox" name="ubah_foto" value="true"> Ceklis jika ingin mengubah Foto
                </div>
              </div>
              <div class="col-md-12">
                <button type="submit" name="edit" class="btn btn-success col-md-2"><i class="fa fa-edit"></i> Edit</button>
                <a href="../datawarkah" class="btn btn-warning col-md-2 col-md-offset-1"><i class="fa fa-refresh"></i> Kembali</a>
              </div>
          </form>
          </div>
        </div>
